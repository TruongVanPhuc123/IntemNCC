...
async function Content_Temp(headers, dataTable, page, tableX, tableY, colWidth, rowHeight, customFont) {
    ...
    headers.forEach((text, colIndex) => {
        ...
        page.drawText(text, {
            x: centerText(tableX, text, colIndex, colWidth, customFont, fontSize),
            y: tableY - 20,
            size: fontSize,
            font: customFont,
        });
    ...
    dataTable.forEach((row, rowIndex) => {
        ...
        // **Tô nền xen kẽ cho từng hàng dữ liệu**
        const backgroundColor = rowIndex % 2 === 0 ? rgb(0.85, 0.85, 0.85) : rgb(0.95, 0.95, 0.95);
